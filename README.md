### Привет, я @NovaStream2030!

Я веб-разработчик из [rsDev](https://vk.com/rsdevxyz). 

Занимаюсь интересными проектами, участвую вместе со своей командой в конкурсах.

- 📱  Я сейчас работаю над VFreelance и другими секретными проектами.
- ~~🤓 Я собираюсь изучить C++ и Rust и мечтаю создавать крутые приложения.~~
- 😒 Не сильно хорошо знаю ReactJS (ну я же копирую у [@gbowsky](https://github.com/gbowsky), надо соответствовать), пишу на TypeScript. Начал потихоньку отходить от JavaScript.
- 📫 Найти меня можно в [VK](https://vk.com/NovaStream2030) и [Telegram](https://t.me/NovaStream2030).
- 🎂 Мне 15 лет!

![Сколько я времени провел за тем или иным языком за неделю. By WakaTime](https://wakatime.com/share/@d7d65ed1-79f1-453d-b2b0-293f15d85800/d1285c5c-50c9-4c2a-a96f-294b583fba12.png)

Честно скопировано у [@gbowsky](https://github.com/gbowsky)
